...
test('it should unshorten custom domain url links', () => {
  tall('http://www.loige.link/codemotion-rome-2017').then(url =>
    expect(url).toBe('https://slides.com/lucianomammino/universal-js-web-applications-with-react-codemotion-rome-2017'),
  );
});

test('it should unshorten bit.ly links', () => {
  tall('http://bit.ly/judo-heroes-tutorial').then(url =>
    expect(url).toBe('https://scotch.io/tutorials/react-on-the-server-for-beginners-build-a-universal-react-and-node-app'),
  );
});
